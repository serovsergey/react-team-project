...
const CustomTaskBox = () => {
    ...
    return(
        <div className={s.box}>
            <p className={s.text}>If you want to get more prizes - add tasks :{")"}</p>
            <button className={s.button} onClick={() => setModalOpen(true)}><FaPlus  color="#fff" size={27} /></button>
            {modalOpen &&
                <Modal onClose={handleCloseModal}>
                    <div className={s.topWrapper}>
                    <button type='button' onClick={handleCloseModal} className={s.closeButton}><CgClose  size={18} className={s.cross}/></button>
                    <label className={s.imageLabel}> <ImageIcon className={s.imageIcon}/>
                    <input type="file" name='image' onChange={handleImageChange} className={s.imageInput} />
                    </label>
